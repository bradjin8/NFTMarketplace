      ...
      <div className={classes.wrapper}>
        ...
        <div style={{width: '90%'}}>
          ...
          }
          {
            window.sessionStorage.getItem('token') &&
            <div>
              <h3 className={classes.userHeader}><span className={classes.transactionTitles}>User:</span> {accountData.account.user}</h3>
          ...
          {window.sessionStorage.getItem('token') && accountData.account.transactions.map((transaction, i) => {
            return(
              <div className={classes.transaction + " transactions"} id={transaction.id}>
                ...
                <img src={`https://crpyotonite-game-site.s3.eu-west-2.amazonaws.com/images/farm/packs/presale-${transaction.pack}-packs.png`} alt=""></img>
                <a>x{transaction.quantity}</a>
                </div>
                <div className={classes.transactionDetails}>
                <ul>
                  <li><a><span className={classes.transactionTitles}>Created at:</span> {transaction.created_at}</a></li>
                  <li><a><span className={classes.transactionTitles}>Value:</span> {transaction.final_price}</a></li>
                  <li><a className={classes.tx_hash} href={`https://bscscan.com/tx/${transaction.tx_hash}`} target="_blank">{transaction.tx_hash}</a></li>
                </ul>
                </div>
